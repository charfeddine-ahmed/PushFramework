...
namespace ProtocolFramework
{
	...
	void AbstractPool::deleteAllocatedMemory()
	{
		for (unordered_set<ObjectPtr>::iterator it = internal->objectsInUse.begin();
			it != internal->objectsInUse.end();
			it++)
		{
			deleteImpl(*it);
		}

		internal->objectsInUse.clear();

		unsigned int size = internal->freeObjects.size();

		for (unsigned int i=0; i<size; i++)
		{
			deleteImpl(internal->freeObjects.at(i));
		}

		internal->freeObjects.clear();
	}
