...
namespace PushFramework
{
	...
	void LogicalConnectionImpl::Recycle()
	{
		...
		{
			getServerImpl().getPhysicalConnectionPool().returnObject(pPhysicalConnection);
			pPhysicalConnection	= NULL;
		}
	}
	...
	SendResult::Type LogicalConnectionImpl::PushPacket( OutgoingPacket* pPacket )
	{
		...
		{
			getServerImpl().getClientFactory().disconnect(this, false, DisconnectionReason::PeerClosure);
		}
		return result;
	}
	...
	bool LogicalConnectionImpl::SubscribeToQueue( BROADCASTQUEUE_NAME queueName, bool ignorePreviousPackets /*= false*/ )
	{
		...
		// Locate the queue.
		BroadcastQueue* pQueue = getServerImpl().getBroadcastManager().getQueue(queueName);
		if (!pQueue)
		{
			return false;
		}
	...
	bool LogicalConnectionImpl::CheckAndProcessPendingBroadcast( bool somethingHasHappened )
	{
		...
		while (pGroup)
		{
			...
			{
				cout << "could not send " << ret << endl;
				if(result == SendResult::NotOK)
				{
					cout << " Fatal error trying to send broadcast message" << endl;
					ret = false;
				}
				break;
			}
	...
	void LogicalConnectionImpl::OnReadyForSend(  )
	{
		return facade->OnReadyForSend();
	}
	...
	{
		getServerImpl().getClientFactory().disconnect(this, waitForPendingPackets, DisconnectionReason::RequestedClosure);
	}

	bool LogicalConnectionImpl::IsInactiveIntern()
	{
		double uInactivityTime = pPhysicalConnection->getTimeToLastReceive();

		if (uInactivityTime > getServerImpl().getServerOptions().uMaxClientIdleTime)
		{
			return true;
		}
	...
	ServerImpl& LogicalConnectionImpl::getServerImpl()
	{
		return pPhysicalConnection->getServerImpl();
	}
