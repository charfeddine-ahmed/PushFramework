...
{
public:
    ClientFactory(ServerImpl* pServerImpl);
    ~ClientFactory(void);
    ...
    void unsubscribeAllFromQueue(BROADCASTQUEUE_NAME queueName);

public:
	void addIPRangeAccess(const char* ipStart, const char* ipStop, bool bPermit);
	bool isAddressAllowed(IN_ADDR	SockAddr);
	bool createPhysicalConnection(SOCKET hSocket, SOCKADDR_IN address, ListenerOptions* pListenerOptions);
