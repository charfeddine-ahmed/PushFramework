...
ClientFactory::ClientFactory(ServerImpl* pServerImpl)
{
...
bool ClientFactory::removeIfExisting( LogicalConnectionImpl* pLogicalConnection )
{
	...
	logicalConnectionSetT::iterator it = clientSet.find(pLogicalConnection);
	if (it != clientSet.end())
	{
		clientSet.erase(it);
...
void ClientFactory::disconnectIntern( LogicalConnectionImpl* pLogicalConnection, bool waitForPendingPackets, int closeReason )
{
	...
	if (!pLogicalConnection->IsMonitor())
	{
		...
		//Statistics :
		pServerImpl->getServerStats().addToCumul(ServerStats::VisitorsHitsOut, 1);
		pServerImpl->getServerStats().addToCumul(ServerStats::VisitorsOnline, -1);
		pServerImpl->getServerStats().addToDuration(ServerStats::VisitorsDuration, pLogicalConnection->getVisitDuration());
	...
	pServerImpl->getGarbageCollector().addDisposableClient(pLogicalConnection);
}
...
void ClientFactory::scrutinizeChannels()
{
	...
	while (it != vectPendingPhysicalConnections.end())
	{
		...
		{
			// Verify expiry : 
			int nMaxDuration = pServerImpl->getServerOptions().uLoginExpireDuration;
...
void ClientFactory::unsubscribeAllFromQueue(BROADCASTQUEUE_NAME queueName)
{
    ScopedLock csLock(cs);
    ...
    while (it != clientSet.end())
    {
        ...
        pClient->UnSubscribeFromQueue(queueName);
